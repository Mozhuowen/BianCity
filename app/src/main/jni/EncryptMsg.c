...
#define LOGE(...){\
	if (1) {\
		__android_log_print(ANDROID_LOG_ERROR,"jni---",__VA_ARGS__);\
	}\
	}
#endif
...
JNIEXPORT
jbyteArray JNICALL Java_com_putaotown_tools_EncryptUtil_getImportantInfoByJNI(
...
jstring Java_com_ldw_aes_utils_AESUtils_getDecrypt(
		JNIEnv *env, jobject thiz, jstring encryptstr) {

//	return getDecrypt(env, encryptstr);
...
//	LOGE("enter getImportInfo method ok!");
	...
	//bacdefghijklmnopqrstuvwxzy1023456798BACDEGF=
	unsigned char key[32] = {0x6D,0xA7,0x1D,0x79,0xF8,0x21,0x8A,0x39,0x25,0x9A,
			0x7A,0x29,0xAA,0xBB,0x2D,0xBA,0xFC,0x31,0xCF,0x2D,0x74,0xDB,0x7E,0x39,
			0xEB,0xBF,0x7C,0x04,0x00,0x83,0x10,0x61};
